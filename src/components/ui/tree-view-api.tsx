...
import { FileTxt, FolderClose, FolderOpen } from '@icon-park/react';
import React, {
  createContext,
  forwardRef,
  useCallback,
    ...
    return (
      <AccordionPrimitive.Item value={id} className="relative">
        <AccordionPrimitive.Trigger
          ...
          aria-label="File"
          className={cn(
            "flex items-center cursor-pointer text-base px-2 box-border rounded-md duration-200 ease-in-out",
            "text-[#00000099]",
            {
        ...
        >
          {fileIcon ?? <FileTxt
            className="h-4 w-4 mr-1 flex justify-center items-center"
            fill="#0000008c"
            strokeWidth={3}
          />}
          <span className="w-full text-left line-clamp-1">{element}</span>
